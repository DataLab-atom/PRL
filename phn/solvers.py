...
def get_Solver(config_dict):
    '''
    attention it !!!!!!!!!!
    config_dict["n_params"] should set in upper loop
    '''
    
    name = config_dict["name"].lower()
    if name in 'ls':
        return LinearScalarizationSolver(n_tasks=config_dict["n_tasks"])
    elif name in 'stchsolver':
        return STCHSolver(n_tasks=config_dict["n_tasks"],mu = config_dict["mu"],Z = config_dict["Z"])
    elif name in 'eposolver':
        return EPOSolver(n_tasks=config_dict["n_tasks"], n_params=config_dict["n_params"])
    elif name == 'sum':
        return sumSolver(n_tasks=config_dict["n_tasks"])
    else :
...
class Solver:
    ...
    def __call__(self, losses, ray, parameters, **kwargs):
        return self.get_weighted_loss(losses, ray * self.n_tasks , parameters, **kwargs)
...
class STCHSolver(Solver):
    """
    https://arxiv.org/abs/2402.19078
    """

    def __init__(self, n_tasks,mu = 0.1,Z = 0.0):
        super().__init__(n_tasks)
        self.mu,self.Z = mu,Z

    def get_weighted_loss(self, losses, ray, parameters=None, **kwargs):
        return losses.sum() + self.mu * torch.logsumexp(losses,0)
