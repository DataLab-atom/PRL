...
class hnet(nn.Module):
    ...
    def __init__(
        ...
        print((target_net_in_features,out_dim))
        self.use_norm = True
        ...
        self.ray = torch.nn.Parameter(torch.from_numpy(
    ...
    def forward(self,_ray=None):
        if _ray == None:
            if self.training:
                ...
                _ray = torch.ones_like(self.ray)
        
        ray = self.ray * torch.nn.functional.softmax(_ray,dim=0)
...
class ResNet_s(nn.Module):
    ...
    def __init__(self, block, num_blocks, num_experts=3, num_classes=10, reduce_dimension=False, layer2_output_dim=None, layer3_output_dim=None, use_norm=False, returns_feat=True, use_experts=None, s=30, use_hnet=False,hnet_feature =False):
        ...
        if self.use_hnet:
            self.hnet2linears = hnet(num_classes,layer3_output_dim,alpha=1.2,use_norm=use_norm)
        else:
            if use_norm:
