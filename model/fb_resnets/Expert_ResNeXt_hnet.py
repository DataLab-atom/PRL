...
from model.ldam_drw_resnets.expert_resnet_cifar_with_hnet import hnet
class ResNext(nn.Module):

    def __init__(self, block, layers, num_experts=3, groups=1, width_per_group=64, dropout=None, num_classes=1000, reduce_dimension=False, layer3_output_dim=None, layer4_output_dim=None, 
                 use_norm = False, returns_feat=False, use_experts=None, s=30, use_hnet=False,hnet_feature=False):
        self.inplanes = 64
        self.num_experts = num_experts
        super(ResNext, self).__init__()
        # print("use_hnet", use_hnet)
        print("use_norm:", use_norm)
        print("reduce_dimension:", reduce_dimension)
        use_norm = True
        ...
        if self.use_hnet:
            print('g')
            self.hnet2linears = hnet(num_classes,layer4_output_dim * block.expansion, alpha=1.2, use_norm=use_norm)
        else:
            if use_norm:
    ...
    def forward(self, x, ray=None):
        ...
        for ind in use_experts:
            ...
            if self.use_hnet:
                hnet_out_dict = self.hnet2linears()
                #--------------------------------------------------------------
                if self.use_norm:
